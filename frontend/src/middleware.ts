...
export async function middleware(req: NextRequest) {
  ...
  /// === EXCLUSÕES DE ARQUIVOS/APIS E ESTÁTICOS ===
  // Permite apenas arquivos JS, CSS, imagens ou rotas de autenticação
  const isStaticFile = pathname.match(/\.(css|js|png|jpg|jpeg|gif|svg|ico)$/i);
  const isNextApi = pathname.startsWith('/api/auth');

  if (isStaticFile || isNextApi) {
